...
void main() {
  testWidgets('keeps the bottom action visible while setup content scrolls', (
    ...
    expect(find.byKey(const Key('app-setup-page-scroll-view')), findsOneWidget);
    expect(find.text('계속'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });

  testWidgets('bounds setup content width on a tablet viewport', (
    tester,
  ) async {
  ...
  testWidgets('shows the current setup step without text labels', (
    ...
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(body: AppSetupHeader(currentStep: 2, totalSteps: 3)),
      ),
    );
...
Color? _stepColor(WidgetTester tester, int step) {
  final indicator = tester.widget<AnimatedContainer>(
    find.byKey(Key('app-setup-step-$step')),
  );

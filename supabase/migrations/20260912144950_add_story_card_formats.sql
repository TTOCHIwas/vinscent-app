alter table public.story_loop_cards
  add column card_type text not null default 'polaroid',
  add column photo_count integer not null default 0;

update public.story_loop_cards
set photo_count = case when has_photo then 1 else 0 end;

create function private.normalize_story_card_format_metadata()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  if new.card_type = 'polaroid' then
...
create trigger normalize_story_card_format_metadata_before_write
before insert or update of card_type, photo_count, has_photo
on public.story_loop_cards
for each row
execute function private.normalize_story_card_format_metadata();

alter table public.story_loop_cards
  add constraint story_loop_cards_card_type_check
    check (card_type in ('polaroid', 'four_cut_grid', 'four_cut_strip')),
  add constraint story_loop_cards_photo_count_check
    check (photo_count between 0 and 4),
...
create function public.upsert_today_story_loop_card_v2(
...
begin
  ...
  if is_four_cut and coalesce(requested_has_caption, false) then
    perform private.raise_app_error('invalid_story_card_caption');
  end if;

  if is_four_cut and nullif(btrim(requested_background_image_path), '') is not null then
...
create function public.get_today_story_card_stacks_v2()
returns table (
  couple_id uuid,
  couple_date date,
  access_mode text,
...
create function public.get_today_story_loop_summary_v2()
returns table (
  couple_id uuid,
  couple_date date,
  access_mode text,
...
begin
  ...
  return query
    ...
    from public.get_today_story_loop_summary() as summary
...
create function public.get_story_loop_detail_v2(target_date date)
returns table (
  couple_id uuid,
  couple_date date,
  access_mode text,
...
begin
  ...
  return query
    select
      ...
      detail.answer_count
    from public.get_story_loop_detail(target_date) as detail
    left join public.story_loop_cards as first_card
...
create function public.get_story_loop_month_summary_v2(target_month date)
returns table (
  couple_date date,
  loop_status text,
  card_count integer,
...
begin
  ...
  return query
    select
      ...
      coalesce(second_card.card_type, 'polaroid'),
      summary.second_card_submitted_at
    from public.get_story_loop_month_summary(target_month) as summary
    left join public.story_loop_cards as first_card
      on first_card.id = summary.first_card_id
    left join public.story_loop_cards as second_card
      on second_card.id = summary.second_card_id;
...
revoke execute on function public.upsert_today_story_loop_card_v2(
  uuid, text, text, text, boolean, boolean, boolean,
  integer, integer, text, integer, boolean, integer
) from public, anon;
revoke execute on function public.get_today_story_card_stacks_v2()
  from public, anon;
revoke execute on function public.get_story_card_stack_v2(date, uuid)
  from public, anon;
revoke execute on function public.get_today_story_loop_summary_v2()
  from public, anon;
revoke execute on function public.get_story_loop_detail_v2(date)
  from public, anon;
revoke execute on function public.get_story_loop_month_summary_v2(date)
  from public, anon;

grant execute on function public.upsert_today_story_loop_card_v2(
  uuid, text, text, text, boolean, boolean, boolean,
  integer, integer, text, integer, boolean, integer
) to authenticated;
grant execute on function public.get_today_story_card_stacks_v2()
  to authenticated;
grant execute on function public.get_story_card_stack_v2(date, uuid)
  to authenticated;
grant execute on function public.get_today_story_loop_summary_v2()
  to authenticated;
grant execute on function public.get_story_loop_detail_v2(date)
  to authenticated;
grant execute on function public.get_story_loop_month_summary_v2(date)
  to authenticated;

alter table public.ai_runs
  ...
  add column provider_error_status text,
  add column provider_retry_after_ms integer;

alter table public.ai_runs
  add constraint ai_runs_provider_http_status_check
    check (
      provider_http_status is null
      or provider_http_status between 100 and 599
    ),
  add constraint ai_runs_provider_error_status_check
    check (
      provider_error_status is null
      or provider_error_status ~ '^[A-Z][A-Z0-9_]{0,99}$'
    ),
  add constraint ai_runs_provider_retry_after_ms_check
    check (
      provider_retry_after_ms is null
      or provider_retry_after_ms between 0 and 86400000
    );
...
create or replace function public.fail_ai_processing_run_with_diagnostics(
...
revoke execute on function public.fail_ai_processing_run_with_diagnostics(
  uuid,
  text,
  text,
  boolean,
...
grant execute on function public.fail_ai_processing_run_with_diagnostics(
  uuid,
  text,
  text,
  boolean,

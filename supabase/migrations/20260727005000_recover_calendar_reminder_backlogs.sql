  ...
  return query
    with reminder_candidates as (
      select
        ...
        (requested_run_at at time zone couple.timezone)::date as local_date,
        extract(
          year from (
            (requested_run_at at time zone couple.timezone)::date
            + reminder.offset_days
          )
        )::integer as target_year
    ...
    due_reminders as (
      ...
      where resolved_occurrences.occurrence_date
        ...
        and resolved_occurrences.occurrence_date
          >= resolved_occurrences.local_date
    ),
    notification_candidates as (
      ...
      from due_reminders
      where due_reminders.scheduled_at < requested_run_at
    )
    select
      candidate.source_id,
      candidate.event_id,

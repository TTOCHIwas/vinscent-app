...
현재 예약 경로는 다음 작업을 함께 처리한다.

- 커플별 설정 시각에 오늘의 질문을 독립적으로 배정하고 전달한다.
- 질문 전달 후 아직 답변하지 않은 사용자의 리마인드를 처리한다.
- 캘린더 일정 리마인드와 실패한 푸시 재시도를 처리한다.
- 자정이 지난 카드 묶음에서 오늘의 카드 또는 최신 카드 한 장을 남기고 나머지를 정리한다. 카드 정리가 실패해도 다른 예약 알림 처리는 계속한다.

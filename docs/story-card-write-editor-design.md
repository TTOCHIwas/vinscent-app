...
- 사진은 카드당 배경 한 장이며, 세로형 9:16 캔버스에서 중앙 채우기를 시작점으로 두 손가락 확대·축소·이동을 지원한다. 이동 결과로 캔버스 일부가 비어도 된다.
...
- 새 카드: 앱 내부 카메라로 시작하며 텍스트 또는 그리기를 선택하면 빈 9:16 캔버스로 진입한다.
...
1. **요구사항과 설계 보완**
   - 9:16 편집 계약, 텍스트 회전·최대 배율, 펜 최대 굵기, 지우개·되돌리기·그리기 완료, 자르기 버튼 제거 후 배경 제스처를 문서화한다.

...
평가 결과에는 합성 출력, 지연 시간과 공급자가 반환한 입출력 토큰 수만
표시하며 API 토큰과 실제 사용자 데이터는 기록하지 않는다.
...
- 각 모델을 세 번 실행했을 때 24개 결과가 모두 계약 검증을 통과한다.
...
Cloudflare는 JSON Mode에서 스키마 준수를 보장하지 않는다고 명시하므로
운영에서도 현재 도메인 검증과 실패 재시도를 유지한다.

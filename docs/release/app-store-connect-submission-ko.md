...
기본 카테고리는 Google Play Console에서 확인한 라이프스타일 분류와
맞춘다. Google Play에 추가 카테고리가 없고 단짠에는 공개 피드나 사용자
탐색도 없으므로 App Store 추가 카테고리는 선택하지 않는다. 인앱 구입과
구독은 현재 출시 후보에 없다.
...
의미를 유지한 짧은 부제를 사용한다. App Store 프로모션 텍스트는 Google
Play 짧은 설명과 정확히 동일하게 유지한다. 전체 설명도 두 스토어에서
글자와 문단까지 동일하게 유지하며 자동 검증기가 차이를 실패로 처리한다.
...
현재 TestFlight 업로드용 App Store Connect API 키는 `Developer` 역할이라
이 앱의 메타데이터를 읽을 수 있지만 수정 요청은 허용되지 않는다. 아래
항목과 제품 페이지 값은 App Store Connect 웹에서 입력한다.

- Apple 연령 등급 재정의 `13+` 또는 `16+` 최종 선택
- Google Play와 동일한 출시 국가 목록 확인
- EU 배포 시 DSA trader 여부와 공개 연락처 검증
- 대한민국 법률 연락처의 이메일·전화번호 인증
- 콘텐츠 권리에서 제3자 콘텐츠 사용과 필요한 권리 보유 확인

...
- Play Console에 140자 이하 대체 텍스트를 함께 입력한다. 제출할 한국어
  원본은 `store-assets/google-play/alt-text.ko.json`에 보관한다.
...
- 추가 홍보 문구 없이 실제 앱 UI를 우선 사용하고 각 이미지에 140자 이하
  대체 텍스트를 입력한다. 휴대전화와 태블릿의 장면별 문구는
  `alt-text.ko.json`의 `phone`·`tablet` 항목을 사용한다.

...
1. 캘린더 첫 표시, 월·주·상세 전환과 이전·다음 달 스와이프가 번쩍임 없이
   이어지는지 확인한다.
2. 오늘 날짜가 브랜드색 채움 원과 흰 글자로 보이고, 오늘이 아닌 선택 날짜는
   검정 채움으로 유지되는지 확인한다.
3. 이전·현재·다음 달 카드·일정 미리보기가 전환 전에 준비되고, 사진 미리보기
   메모리 사용이나 네트워크 요청이 비정상적으로 늘지 않는지 확인한다.
4. 그림·텍스트 팔레트, 스포이드, 가로 굵기 조절기, 하단 도구 배치가 Safe
   Area와 큰 글자에서 겹치지 않는지 확인한다.
5. 공통 확인창의 세로 행동 버튼과 단어 단위 줄바꿈이 작은 화면과 큰 글자에서
   잘리지 않는지 확인한다.
6. 홈·캘린더·AI·녹음 화면의 업데이트 인디케이터가 읽은 뒤 사라지고 부모와
   자식 화면에서 같은 상태를 나타내는지 확인한다.
...
기기 캘린더의 Dart coordinator 테스트는 존재하지만, 현재
`apps/mobile/ios/RunnerTests/RunnerTests.swift`는 예제 상태다. EventKit 문제가
발견되면 문자열 검사만 늘리지 말고 재현 가능한 XCTest 또는 테스트 가능한
네이티브 경계를 먼저 추가한 뒤 수정한다.
...
최종 보고에는 HEAD SHA, 사용한 Xcode·SDK·Flutter 버전, 자동 테스트 결과,
시뮬레이터와 실제 기기별 검증 결과, 남은 차단 사항을 포함한다.

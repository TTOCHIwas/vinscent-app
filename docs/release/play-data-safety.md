...
검토 기준일은 2026년 8월 3일이다. 릴리스 후보의 권한, SDK, 서버 처리
...
쉽게 찾을 수 있는 `/account-deletion` 페이지를 공개해야 한다. 정책 웹
문구와 릴리스 빌드는 준비됐지만 공개 URL 배포 전이므로 이 조건은 아직
충족하지 않는다.
...
- 외부 요청은 로그인 방식, 닉네임과 등록 이메일을 확인하고 등록 이메일로
  보낸 일회용 코드가 확인된 날부터 10일 이내에 처리 결과를 같은 문의
  대화로 알린다.
- AI·알림 상세 진단은 90일, 검토 완료 신고와 관련 기록은 검토일부터
  1년 보관한다. 미처리 신고와 재시도 가능한 작업은 기간 정리에서 제외한다.

## 7. Console 입력 전 차단 조건

- [x] 법적 운영자명과 개인정보 문의 이메일 확정

...
| 신고 운영 알림 | 신고 ID, 대상 종류, 사유 분류, 신고 시각, 상세 설명·스냅샷 존재 여부 | 새 신고의 운영 검토 요청과 중복 방지 | 비공개 Discord 안전 신고 채널 |
...
`20260803000000_enforce_operational_data_retention.sql`은 매일 03:17 KST에
서버 내부 정리 함수를 실행한다. 처리 중이거나 재시도 가능한 작업과 미처리
신고는 기간 정리 대상에서 제외한다.

| 기록 | 자동 파기 | 계정·커플 삭제 시 동작 | 보존 경계 |
|---|---|---|---|
| `ai_processing_jobs` | 완료·실패·취소 후 90일이 지난 작업 삭제 | 커플 삭제 시 연쇄 삭제 | 대기·처리 중 작업 유지 |
| `ai_runs` | 90일 후 참조 없는 실행 삭제, 기능 결과가 참조하는 실행은 상세 진단만 제거 | 커플 삭제 시 연쇄 삭제 | 참조 실행은 작업 종류·공급자·모델·프롬프트 버전·처리 및 안전 상태만 유지 |
| `push_notification_dispatches` | 전송 완료 또는 재시도 소진 후 90일이 지난 기록 삭제 | 수신자 계정 삭제 시 연쇄 삭제 | 처리 중·재시도 가능 기록 유지 |
| `push_notification_deliveries` | 90일이 지난 기록 중 처리 중·재시도 가능 dispatch에 필요하지 않은 기록 삭제 | 수신자 ID는 `null`로 바뀔 수 있음 | 활성 재시도 근거 유지 |
| `safety_reports` | 검토 완료일부터 1년 후 삭제 | 신고자 계정 삭제 시 연쇄 삭제 | 미처리 신고는 검토 완료까지 유지 |
| `safety_report_reviews`, `safety_moderation_alerts` | 원본 신고와 함께 삭제 | 원본 신고 삭제 시 연쇄 삭제 | 원본 신고와 동일한 수명 적용 |
...
- 대표자 조준희와 공개 문의 이메일을 정책에 표시한다.
- 정책 시행일은 2026년 8월 3일이다.
- AI·알림 상세 진단은 90일, 검토 완료 신고는 검토일부터 1년 보관한다.
- 미처리 신고는 검토 완료까지 보관하며, 신고자가 계정을 삭제하면 현재
  계정 삭제 경계에 따라 함께 제거한다.
- 신고 운영 담당자는 대표자 조준희이고 최초 검토 목표는 접수 후 7일이다.
- 외부 계정 삭제는 등록 이메일 일회용 코드를 확인하고 확인 완료일부터
  10일 이내에 같은 문의 대화로 결과를 알린다.
- 외부 처리자별 계약상 역할, 국외 이전 리전·시점·방법과 처리자 보유기간은
  스토어 설문을 제출하기 전에 계약 문서와 운영 계정에서 별도로 확인한다.
...
- 서비스 가입과 이용은 만 14세 이상으로 제한한다. 앱은 생일 선택 단계에서
  ...
  우회 저장을 차단한다.
- AI 실행, 푸시 발송과 신고 기록의 기간 기반 자동 정리 migration이
  구현돼 있으며 운영 Database 적용과 첫 실행 결과 확인이 남아 있다.
...
- Google Play의 1:1 UGC 요구에 필요한 안전 약속 동의, 신고와 사용자
  차단 경계는 구현돼 있다. Apple Guideline 1.2의 게시 전 필터링 방법은
  iOS 제출 전에 별도로 확정해야 한다.
- 신고 모더레이션 알림 큐와 최소 메타데이터 전송 워커, 비공개 Discord
  채널, 담당자와 최초 검토 목표가 정해졌다. 운영 배포 후 실제 수신과
  재시도 증빙 확인은 남아 있다.
...
- `supabase/migrations/20260803000000_enforce_operational_data_retention.sql`

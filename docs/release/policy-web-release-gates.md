...
| 신고 검토 기반 | 내구성 있는 모더레이션 알림 큐, 최소 메타데이터를 보내는 비공개 Discord 채널과 검토자·결정 감사 경계가 구현됨 | 실제 수신·재시도 검증과 담당자·목표 시간 결정 필요 |
| 개인정보 저장 위치 | Supabase 프로젝트의 주 데이터 리전은 `ap-northeast-1`임 | 연결된 프로젝트 환경 |
| 운영자 정보 | 공개 문의 이메일은 정책 웹에 반영됐고 법적 운영자명과 시행일은 미확정 | 운영자 확인 필요 |
...
| 외부 삭제 접수 | 공개 이메일 접수 경로는 구현됐고 본인 확인 방법, 처리 기한과 완료 통지 절차가 미확정 | `apps/policy-web/app/account-deletion/page.tsx` |
| 고객지원 웹 경로 | `/support`에 공개 이메일이 연결됐고 운영 응답 기준은 미확정 | `apps/policy-web/app/support/page.tsx` |
...
5. 신고 검토 목표 시간과 운영 담당자
6. 외부 계정 삭제 요청의 본인 확인 방법
7. 외부 계정 삭제 요청의 처리 기한과 완료 통지 방법

...
- [ ] GitHub `supabase-production` Environment에 승인자, `main` branch
  제한, `SUPABASE_ACCESS_TOKEN`, `SUPABASE_DB_PASSWORD`,
...
- [ ] 1:1 UGC의 안전 약속 동의, 신고·차단과 운영 조치 흐름을 review note에 설명
...
파일 규격, 촬영 상태와 개인정보 제거 기준은
`docs/release/store-creative-assets.md`를 따른다.
...
- [ ] 의존성 해석 후와 최종 요약 전 source commit·worktree 검증 통과
- [ ] 카테고리와 tag 선택
...
- [ ] App Review Guideline 1.2의 부적절한 UGC 필터링 방법을 확정하고
  실제 앱 동작과 review note를 일치시킴
